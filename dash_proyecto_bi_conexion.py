...
app.layout = html.Div([
    ...
    # ---------------------------------------------------------------- Fila 3: Tipo Intervención + Departamentos (Beneficiarios)
    html.Div([
    ...
    # ---------------------------------------------------------------- Fila 4: Departamentos por Monto Viable
    html.Div([dcc.Graph(id='departamento-monto-chart')], style={'marginTop': '20px'})
])
...
@app.callback(
    [Output('kpi-cards', 'children'),
     ...
     Output('departamento-monto-chart', 'figure')],
...
def update_dashboard(sectores, niveles, estados, entidades, start_date, end_date):
    ...
    # ---------- 6. Top 10 Departamentos por Monto Viable
    depto_monto_data = dff.groupby('departamento')['monto_viable'].sum().reset_index().nlargest(10, 'monto_viable')
    fig_depto_monto = px.bar(depto_monto_data, x='monto_viable', y='departamento', orientation='h',
                              title='Top 10 Departamentos por Monto Viable',
                              color='monto_viable', color_continuous_scale='Greens',
                              labels={'monto_viable': 'Monto Viable (S/)', 'departamento': ''})
    fig_depto_monto.update_layout(yaxis={'categoryorder': 'total ascending'})

    return kpis, fig_nivel, fig_sector, fig_evolucion, fig_tipo, fig_depto, fig_depto_monto
